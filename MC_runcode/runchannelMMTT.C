#define runchannelMMTT_cxx
#include "runchannelMMTT.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <cmath>
void runchannelMMTT::Loop(const std::string SampleName,const int maxEvents)
{
//   In a ROOT session, you can do:
//      Root > .L runchannelMMTT.C
//      Root > runchannelMMTT t
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   runchannelMMTT::MMTT_PartInfo aParticle;
   ...
   cout<<"RUNNING SAMPLE "<<SampleName<<" runchannel CHANNEL MMTT"<<endl;
   TString fullName = SampleName+"MMTT.root";
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if (eVetoAZHdR0 !=0 || muVetoAZHdR0>2) continue;
      if (doubleMuPass <= 0) continue;

      if(abs(m1Eta) >= 2.4 || abs(m1PVDXY)>=0.045 || abs(m1PVDZ)>=0.2 || m1_t1_DR <= 0.5 || m1_t2_DR <= 0.5) continue;
      if(abs(m2Eta) >= 2.4 || abs(m2PVDXY)>=0.045 || abs(m2PVDZ)>=0.2 || m2_t1_DR <= 0.5 || m2_t2_DR <= 0.5) continue;
      ...
      if(abs(t2Eta) >= 2.3  || t2Pt <= 19 || abs(t2PVDZ)>=0.2 || t2DecayModeFinding !=1 || t2AgainstElectronVLooseMVA6 !=1 || t2AgainstMuonLoose3 !=1 || abs(t2Charge) != 1) continue;
       ...
       if (!(MMTT_ZMMSelection(aParticle))) continue; 
  ...
  // leptons var
     ...
     costhetastar_1_3 =m1_t1_CosThetaStar;
     costhetastar_1_4 =m1_t2_CosThetaStar;
     costhetastar_2_3 =m2_t1_CosThetaStar;
     costhetastar_2_4 =m2_t2_CosThetaStar;
     ...
     MvaMetCovMatrix00_1_3= m1_t1_MvaMetCovMatrix00;
     MvaMetCovMatrix01_1_3= m1_t1_MvaMetCovMatrix01;
     MvaMetCovMatrix10_1_3= m1_t1_MvaMetCovMatrix10;
     MvaMetCovMatrix11_1_3= m1_t1_MvaMetCovMatrix11;
     
     MvaMetCovMatrix00_1_4= m1_t2_MvaMetCovMatrix00;
     MvaMetCovMatrix01_1_4= m1_t2_MvaMetCovMatrix01;
     MvaMetCovMatrix10_1_4= m1_t2_MvaMetCovMatrix10;
     MvaMetCovMatrix11_1_4= m1_t2_MvaMetCovMatrix11;
     
     MvaMetCovMatrix00_2_3= m2_t1_MvaMetCovMatrix00;
     MvaMetCovMatrix01_2_3= m2_t1_MvaMetCovMatrix01;
     MvaMetCovMatrix10_2_3= m2_t1_MvaMetCovMatrix10;
     MvaMetCovMatrix11_2_3= m2_t1_MvaMetCovMatrix11;
       
     MvaMetCovMatrix00_2_4= m2_t2_MvaMetCovMatrix00;
     MvaMetCovMatrix01_2_4= m2_t2_MvaMetCovMatrix01;
     MvaMetCovMatrix10_2_4= m2_t2_MvaMetCovMatrix10;
     MvaMetCovMatrix11_2_4= m2_t2_MvaMetCovMatrix11;
     ...
     PFChargedIso_3 = -9999.0;
     PFNeutralIso_3 = t1NeutralIsoPtSum;
     PFPUChargedIso_3 = -9999.0;
     PFPhotonIso_3 = -9999.0;
...
bool runchannelMMTT::MMTT_ZMMSelection(MMTT_PartInfo aParticle)
{
